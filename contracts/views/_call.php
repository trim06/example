...
<div class="row" style="margin: 20px 0;">
	<?php $form = ActiveForm::begin(['id' => 'meetForm', 'action' => rtrim('/contracts/task-call/'.$taskForm->getContract()->primaryKey.'/'.$taskForm->getTask()->primaryKey, '/'), 'enableClientValidation' => FALSE, 'fieldClass' => 'common\extensions\widgets\SmartActiveField', 'options' => ['class' => 'smart-form', 'data-id' => $taskForm->getContract()->primaryKey]]); ?>
	<div class="col-xs-8 col-sm-9">
		<div class="piece-of-sheet">
			<div class="row">
				<div class="col-xs-12 col-sm-6">
					<?= $this->render('_task_datetimepicker', ['form' => $form, 'taskForm' => $taskForm, 'setInterval' => true]) ?>
				</div>
				<div class="col-xs-12 col-sm-6">
	...
	<div class="col-xs-3 list-actions">
		<div class="top-actions">
			<?php if ($taskForm->getTask()->isNewRecord): ?>
				<?= Html::submitButton('Перезвонить', ['class' => 'btn btn-block btn-sm btn-primary', 'onclick' => 'return contracts.save(this);']); ?>
		...
		<?php if (!$taskForm->getTask()->isNewRecord): ?>
		<div class="top-actions">
			<?= Html::button('Встреча', ['class' => 'btn btn-block btn-sm btn-primary aside-ajax', 'aside-url' => ("/contracts/task-meet/" . $taskForm->contract->id), 'aside-title' => $taskForm->contract->abonent->name . ' ' . \FormatText::phone($taskForm->contract->abonent->phone), 'aside-width' => 700]); ?>
		</div>
		<?php endif; ?>
		<?php if (!$taskForm->getTask()->isNewRecord): ?>
